...
func syncDevicesMap(p devicesMapSyncParams, ctx context.Context) error {
	...
	last := map[uint32]devicesmap.DeviceState{}
	initialized := false
	...
	for {
		...
		if !initialized {
			// the following cleanup can be relevant after agent restart
			pruneStaleDevices(p, desired)
			initialized = true
		}

		upsertDesiredDevices(p, last, desired)
		pruneMissingDevices(p, last, desired)
		last = desired
...
func upsertDesiredDevices(p devicesMapSyncParams, last, desired map[uint32]devicesmap.DeviceState) {
	for ifindex, state := range desired {
		if prev, ok := last[ifindex]; ok && prev == state {
			continue
		}
...
func pruneMissingDevices(p devicesMapSyncParams, last, desired map[uint32]devicesmap.DeviceState) {
	for ifindex := range last {
		if _, ok := desired[ifindex]; ok {
			continue
		}
		if err := p.DeviceMap.Delete(ifindex); err != nil {
			p.Logger.Warn("Failed to delete device map entry",
				logfields.Error, err,
				logfields.Interface, ifindex,
			)
		}
